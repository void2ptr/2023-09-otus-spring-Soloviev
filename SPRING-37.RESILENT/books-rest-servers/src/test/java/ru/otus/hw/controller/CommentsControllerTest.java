...
class CommentsControllerTest {
    ...
    void shouldReturnCommentsForBook(Comment comment) throws Exception {
        ...
        mockMvc.perform(get("/api/v1/books/" + commentsDto.getBook().getId() + "/comments"))
    ...
    @ParameterizedTest()
    @ArgumentsSource(CommentsArgumentsProvider.class)
    void insertAction(Comment comment) throws Exception {
        // init
        ...
        // method for test
        mockMvc.perform(post("/api/v1/books/comments")
    ...
    void updateAction(Comment comment) throws Exception {
        ...
        // method for test
        mockMvc.perform(put("/api/v1/books/comments")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteAction(Comment comment) throws Exception {
        ...
        // method for test
        mockMvc.perform(delete("/api/v1/books/comments/" + comment.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .param("commentId", String.valueOf(comment.getId()))
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
